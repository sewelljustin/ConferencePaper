...
\newcommand{\forAP}{for each available AP}
\begin{algorithm}
...
Displacement of the user is estimated by calculating the length of the current step. BluNavi's DR model uses measurements provided by an accelerometer to calculate step length. The values are used in the formula presented in [14] which can be seen in Eq. 1
\begin{equation}
s_n = K\sqrt[4]{a_n^{max} - a_n^{min}}
\end{equation}
where $n$ is the current step, $s_n$ is the length of step $n$, and $a_n^{max}$ and $a_n^{min}$ are the maximum and minimum values of the accelerometer's vertical axis during step $n$, respectively.
The values used are derived from raw accelerometer data that is passed into a low-pass filter to reduce noise and smooth the function’s peaks and valleys. Orientation, which will be converted to an azimuth angle, is tracked using a gryoscope and magnetometer.
