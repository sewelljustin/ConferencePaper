...
The dead reckoning model used has two key components: displacement and orientation. Displacement can be determined by estimated step length after each step. [14] presented an easy to implement formula [add equation]. The values used are derived from raw accelerometer data that is passed into a low-pass filter to reduce noise and smooth the function’s peaks and valleys. Orientation, which will be converted to an azimuth angle, is tracked using a gryoscope and magnetometer.
